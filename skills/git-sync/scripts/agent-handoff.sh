...
\`\`\`powershell
New-Item -ItemType Directory -Force -Path "$LOCAL_PARENT" | Out-Null   # create when missing; never clears existing content
cd $LOCAL_PARENT
...
\`\`\`
   ...
   folder    : $FOLDER   (under $LOCAL_PARENT - a NEW folder; never overwrite an existing clone)
