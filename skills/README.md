...
本目录另有 2 个**外部桥技能**（登记在 `.external-skills`，来自 BrowserSkill 分支 `arena/01a0b237-browserskill`）：`git-sync` v2.9.2 负责沙箱 ↔ 本机 Windows 的 git 打通与值守，`arena-local-bridge` v2.0 是选装的会话编排。它们按上游原样保留，不参与 light-* 的命名校验；用法见 `../docs/local-bridge/README.md`。
